...
class SDG(nn.Module):
    ...
    def forward(self, local_feat, coarse, f_g, partial):
        batch_size, _, N = coarse.size()  # b 3 512
        f = self.conv_x1(self.relu(self.conv_x(coarse)))  # b 128 512
        f_g = self.conv_1(self.relu(self.conv_11(f_g)))  # b 128 1
        F = torch.cat([f, f_g.repeat(1, 1, f.shape[-1])], dim=1)  # b 256 512

        local_feat = self.mlpp(local_feat)  # b 768 512
        feat_fine, F_H = self.refine_1(local_feat, F)  # b 512 512, b 512 512
        ...
        F_L = self.conv_delta(self.conv_ps(torch.cat([feat_fine, F_H], 1)).reshape(batch_size, -1, N*self.ratio))  # B 128 2048
        O_L = self.conv_out(self.relu(self.conv_out1(F_L)))  # B 3 2048
...
class SVFNet(nn.Module):
    ...
    def forward(self, points, depth, image):
        ...
        x2_d = (self.sa(x)).reshape(batch_size, self.channel*4, 256)  # B 256 256
...
class Model(nn.Module):
    ...
    def forward(self, partial, depth, image):
        ...
        feat_g, coarse = self.encoder(partial, depth, image)  # B 512 1, B 3 256
