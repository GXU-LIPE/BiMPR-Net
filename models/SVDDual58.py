...
from encoder_image.resnet_58 import ResNet
...
class SDG(nn.Module):
    def __init__(self, channel=128, ratio=1, hidden_dim=512):
        ...
        self.cross_attn1 = nn.MultiheadAttention(256, 4, batch_first=True)
        self.layer_norm1 = nn.LayerNorm(256)

    def forward(self, local_feat, coarse, f_g, partial, f_di):
        ...
        local_feat = local_feat.permute(0, 2, 1)  # B 512 256
        f_di = f_di.permute(0, 2, 1)  # B 196 256

        x, _ = self.cross_attn1(local_feat, f_di, f_di)
        local_feat = self.layer_norm1(x + local_feat)  # B x N x F(B 512 256)
        local_feat = local_feat.permute(0, 2, 1)  # B 256 512
...
class SVFNet(nn.Module):
    ...
    def forward(self, points, depth, image):
        ...
        f_i, f_di = self.im_encoder(image)
        f_i = f_i.view(batch_size, 1, -1).transpose(1, 2).contiguous()
...
class Model(nn.Module):
    ...
    def forward(self, partial, depth, image):
        ...
        feat_g, coarse, f_di = self.encoder(partial, depth, image)
        ...
        fine1 = self.refine(local_feat, coarse_merge, feat_g, partial, f_di)
