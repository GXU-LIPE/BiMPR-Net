...
class Feedforward(nn.Module):
    def __init__(self, d_model_out=512, dim_feedforward=1024, dropout=0.0):
        super().__init__()
        self.linear11 = nn.Linear(d_model_out, dim_feedforward)
        self.dropout1 = nn.Dropout(dropout)
        self.linear12 = nn.Linear(dim_feedforward, d_model_out)
...
class PCT_refine(nn.Module):
    def __init__(self, channel=128, rate=1):
        ...
        self.sa4 = cross_transformer(768, 768)
        self.sa5 = cross_transformer(768, 512)
        self.sa6 = cross_transformer(512, channel * rate)
    ...
    def forward(self, local_feat, Feat):
        ...
        y1 = self.sa1(Feat, Feat)  # B 768 512
        y2 = self.sa4(local_feat, local_feat)
...
class SDG(nn.Module):
    ...
    def forward(self, local_feat, coarse, f_g, partial):
        ...
        feat_fine, F_H = self.refine(local_feat, F)
        feat_fine1 = self.ffn(feat_fine)
        F_H1 = self.ffn(F_H)
        ...
        F_L = self.conv_delta(self.conv_ps(torch.cat([feat_fine1, F_H1], 1)).reshape(batch_size, -1, N*self.ratio))
...
class local_encoder(nn.Module):
    def __init__(self):
        ...
        self.ffn_1 = Feedforward()

    def forward(self, input):
        x1 = self.gcn_1(input)
        idx = furthest_point_sample(input.transpose(1, 2).contiguous(), self.local_number)
        x_g1 = gather_points(x1, idx)

        x_1 = self.sa1(x_g1, x1)
        x_1 = self.ffn_1(x_1)
        x_1 = torch.cat([x_g1, x_1], dim=1)
