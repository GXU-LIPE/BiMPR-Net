...
class Decoder(nn.Module):
    def __init__(self, hidden_dim, channel, ratio):
        super().__init__()
        self.sa1 = self_transformer(hidden_dim, hidden_dim, dropout=0.0, nhead=8)
        self.sa2 = self_transformer(hidden_dim, channel * ratio, dropout=0.0, nhead=8)
...
class SDG(nn.Module):
    def __init__(self, channel=128, ratio=1, hidden_dim=512):
        ...
        self.cross1 = cross_transformer(hidden_dim, hidden_dim, dropout=0.0, nhead=8)

        self.decoder1 = SDG_Decoder(hidden_dim, channel, ratio)
        self.decoder = Decoder(hidden_dim, channel, ratio)
    ...
    def forward(self, local_feat, coarse, f_g, partial):
        ...
        feat_corse, feat_fine = self.refine(None, f, F)
