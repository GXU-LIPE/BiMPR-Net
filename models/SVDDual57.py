...
class cross_transformer(nn.Module):
    ...
    def forward(self, src1, src2, pos, if_act=False):
        ...
        src1 = self.with_pos_embed(src1, pos)
...
class Refinement(nn.Module):
    def __init__(self, dim_1=256, dim=768):
        ...
        self.sa1 = cross_transformer(dim_1, dim_1)
        self.sa2 = cross_transformer1(dim_1, dim, dropout=0.0, nhead=8)
        self.sa3 = cross_transformer1(dim_1, dim, dropout=0.0, nhead=8)
        self.cpe = ConvPosEnc(dim_q=dim_1)
        self.cross1 = cross_transformer(dim_1, dim_1, dropout=0.0, nhead=8)

        self.pos_embed = nn.Sequential(
            nn.Conv1d(3, 128, 1),
            nn.BatchNorm1d(128),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(128, dim_1, 1)
        )
    ...
    def forward(self, local_feat, Feat, coarse, part_cloud):
        ...
        emd_coarse = self.pos_embed(coarse)
        emd_part_cloud = self.pos_embed(part_cloud)

        y1 = self.sa1(Feat, Feat, emd_coarse)
        y2 = self.sa2(y1, y1)

        x = self.cpe(q=y1)
        F_H = self.cross1(x, local_feat, emd_part_cloud)
...
class SDG(nn.Module):
    def __init__(self, channel=128, ratio=1, hidden_dim=512):
        ...
        self.refine_1 = Refinement(dim_1=256, dim=768)
        self.refine_2 = Refinement(dim_1=768, dim=512)
        self.refine_3 = Refinement(dim_1=512, dim=512)
        # self.cpe = ConvPosEnc(dim_q=768, dim_content=1)

    def forward(self, local_feat, coarse, f_g, part_cloud):
        ...
        feat_fine, F_H = self.refine_1(local_feat, F, coarse, part_cloud)
        feat_fine, F_H = self.refine_2(F_H, feat_fine, coarse, part_cloud)
        feat_fine, F_H = self.refine_3(F_H, feat_fine, coarse, part_cloud)
        ...
        F_L = self.conv_delta(self.conv_ps(torch.cat([feat_fine, F_H], 1)).reshape(batch_size, -1, N*self.ratio))
...
class local_encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        point1 = gather_points(input, idx)
        x2 = self.gcn_2(x1)
...
class Model(nn.Module):
    ...
    def forward(self, partial, depth, image):
        ...
        local_feat, part_cloud = self.localencoder(partial)
        ...
        fine1 = self.refine(local_feat, coarse_merge, feat_g, part_cloud)
