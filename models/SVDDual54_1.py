...
class SVFNet(nn.Module):
    ...
    def forward(self, points, image):
        ...
        # single_view_feature_1 = self.posmlp_1(single_view).permute(2, 0, 1)  # 1 B 128
        # f_v = self.attn(torch.cat([f_v, f_i], 1), single_view_feature_1)  # B 128 1

        # f_v_ = self.viewattn(torch.cat([f_v, f_p], 1))  # B 256 1
        f_v_ = self.viewattn(torch.cat([f_i, f_p], 1))  # B 256 1
        f_v_ = F.adaptive_max_pool1d(f_v_, 1)  # B 256 1
...
class Model(nn.Module):
    ...
    def forward(self, partial, image):
        ...
        feat_g, coarse = self.encoder(partial, image)
