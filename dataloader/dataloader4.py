...
class ViPCDataLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if (len(key.split(';')[-1]) > 3):
            ...
            print(key.split(';')[-1])
            fin = key.split(';')[-1][-2:]
            interm = key.split(';')[-1][:-2]

            pc_path = os.path.join(self.gt_path,
                                   key.split(';')[0] + '/' + interm + '/' + fin.replace('\n', '') + '.dat')
        ...
        views_data = []  # 存储所有视图的数据
        for view_index in range(24):
            ran_key = key[:-3] + str(view_index).rjust(2, '0')
            ...
            views_data.append((views.float(), torch.from_numpy(pc).float(), torch.from_numpy(pc_part).float(), image_path))

        return views_data  # 返回所有 24 幅视图的数据列表
...
if __name__ == "__main__":
    ...
    category = "table"
    ViPCDataset = ViPCDataLoader('test_list2.txt', data_path='/home/aiello/ShapeNetViPC-Dataset', status='test',
                                 category=category)
