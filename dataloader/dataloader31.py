...
class ViPCDataLoader(Dataset):
    def __init__(self, filepath, data_path, status, pc_input_num=3500, view_align=True, category='all'):
        ...
        for key in self.filelist:
            if category != 'all':
                if key.split(';')[0] != self.cat_map[category]:
                    continue
            # print(key.split(';')[0])
            # if key.split(';')[0] in ['02691156', '02933112', '02958343', '03001627', '03636649', '04256520', '04379243', '04530566']:
    ...
    def __getitem__(self, idx):
        ...
        return views.float(), torch.from_numpy(pc).float(), torch.from_numpy(pc_part).float(), image_path, ran_key
