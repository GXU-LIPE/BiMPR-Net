...
from models.SVDDual54_fps import Model
...
parser.add_argument('--partial_point', type=int, default=2048)
parser.add_argument('--test_category', type=str, default='plane')
parser.add_argument('--Test_record_path', type=str, default='/data/home-gxu/ly21/XMFnet/log/BiMPR-Net/plane')
...
# checkpoint = os.path.join(args.Test_record_path, 'best_model_all.pt')
...
# def random_drop_points(partial, num_points_to_drop):
#     B, N, C = partial.shape
#     indices_to_drop = torch.randperm(N)[:num_points_to_drop]
#     partial = partial[:, indices_to_drop, :]
#     return partial
#
#
# def random_dropout(point_cloud, num_points_to_drop):
#     batch_size, num_points, dimensions = point_cloud.shape
#     drop_indices = np.random.choice(num_points, num_points_to_drop, replace=False)
#     processed_point_cloud = np.delete(point_cloud.cpu(), drop_indices, axis=1)
#
...
with torch.no_grad():
    ...
    for data in tqdm(test_loader, colour='red', desc='Test Process'):
        ...
        partial = farthest_point_sample(partial, args.partial_point)
        gt = farthest_point_sample(gt, 2048)
        gt1 = farthest_point_sample(gt, 1024)
        gt2 = farthest_point_sample(gt, 512)
        gt3 = farthest_point_sample(gt, 256)
        # partial = random_dropout(partial, 1792).to(device)
        ...
        vis_gt_path_1 = os.path.join(category_path, '真值点云1')
        if not os.path.exists(vis_gt_path_1):
            os.mkdir(vis_gt_path_1)
            print('成功创建真值点云1文件夹')

        vis_gt_path_2 = os.path.join(category_path, '真值点云2')
        if not os.path.exists(vis_gt_path_2):
            os.mkdir(vis_gt_path_2)
            print('成功创建真值点云2文件夹')

        vis_gt_path_3 = os.path.join(category_path, '真值点云3')
        if not os.path.exists(vis_gt_path_3):
            os.mkdir(vis_gt_path_3)
            print('成功创建真值点云3文件夹')
        ...
        save_gt_points_1 = visualize_point_cloud(gt1, vis_gt_path_1, index, ran_key)
        save_gt_points_2 = visualize_point_cloud(gt2, vis_gt_path_2, index, ran_key)
        save_gt_points_3 = visualize_point_cloud(gt3, vis_gt_path_3, index, ran_key)
        # 将输入、输出和真值点云存储为ply文件
