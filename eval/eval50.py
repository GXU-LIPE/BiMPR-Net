...
from models.SVDDual50 import Model
...
##### 运行测试程序时记得修改下列配置
test_category = 'cabinet'
Test_record_path = '/data/home-gxu/ly21/XMFnet/log/models_svd_dual6/models_svd_dual6_0.1_192_all_train36_Wed Oct 25 01:57:15 2023/Test_record'
...
logging.basicConfig(filename=os.path.join(Test_record_path, f'test_{test_category}.log'), filemode='a+', level=logging.INFO, format=log_format)
checkpoint = os.path.join(Test_record_path, 'best_model_cabinet.pth')
...
with torch.no_grad():
    ...
    for data in tqdm(test_loader, colour='red', desc='Test Process'):
        ...
        category_path = os.path.join(vis_path, f'{test_category.capitalize()}')
        if not os.path.exists(category_path):
            os.mkdir(category_path)
            print(f'成功创建{test_category}类别文件夹')
